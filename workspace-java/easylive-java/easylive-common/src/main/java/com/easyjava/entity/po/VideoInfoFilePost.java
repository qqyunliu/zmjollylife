...
public class VideoInfoFilePost implements Serializable {
    ...
    public String toString() {
        return "唯一ID:" + (fileId == null ? "空" : fileId) + ",上传ID:" + (uploadId == null ? "空" : uploadId) + ",用户ID:" + (userId == null ? "空" : userId) + ",视频ID:" + (videoId == null ? "空" : videoId) + ",文件索引:" + (fileIndex == null ? "空" : fileIndex) + ",文件名:" + (fileName == null ? "空" : fileName) + ",文件大小:" + (fileSize == null ? "空" : fileSize) + ",文件路径:" + (filePath == null ? "空" : filePath) + ",更新时间:" + (updateTime == null ? "空" : DateUtils.format(updateTime, DateTimePatternEnum.YYYY_MM_DD_HH_MM_SS.getPattern())) + ",0:无更新 1:有更新:" + (transferResult == null ? "空" : transferResult) + ",持续时间（秒）:" + (duration == null ? "空" : duration) + ",0:转码中 1:转码成功 2:转码失败:" + (postType == null ? "空" : postType);
    }
