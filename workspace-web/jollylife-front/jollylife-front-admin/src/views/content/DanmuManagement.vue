  ...
  <div class="management-container">
    ...
    <div class="management-content">
      ...
      <el-table :data="list" style="width: 100%" v-loading="loading">
        <el-table-column prop="danmuId" label="弹幕ID" min-width="8%" />
        <el-table-column prop="content" label="弹幕内容" min-width="25%" show-overflow-tooltip />
        <el-table-column prop="nickName" label="发布者" min-width="12%">
...
import { loadDanmuApi, delDanmuApi } from '@/utils/Api'

export default {
  name: 'DanmuManagement',
  setup() {
    const list = ref([])
    const total = ref(0)
    ...
    const getList = async () => {
      ...
      try {
        ...
        const response = await loadDanmuApi(params)
    ...
    const deleteItem = async (row) => {
      try {
        ...
        const response = await delDanmuApi({ danmuId: row.danmuId })
        if (response.code === 200) {
